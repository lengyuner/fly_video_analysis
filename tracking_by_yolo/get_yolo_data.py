...
def creat_txt_for_yolov5(position_np,
                         images_folder='H:/data/fly_coco_yolo_4',
                         label_path='H:/tracking/yolov5/data/fly/labels/train2017',
                         ):

    img_index = 0
    K_5 = 0
    for img_name in os.listdir(images_folder):
        label_name = os.path.join(label_path, img_name.replace('.jpg', '.txt'))

        fd = open(label_name, "w")
        # f = open("Label.txt", 'a')
        # phase = 'coco'
        # img_name = 'fly_' + phase + '_' + str(K_2).zfill(12) + '.jpg'
        filename = os.path.join(images_folder, img_name)
        ...
        for K_6 in range(7):
            ...
            # line = line + str(int(x1)) + "," + str(int(y1)) + "," + str(int(bh)) + "," + \
            #        str(int(bw)) + "," + str("0") + " "
            line = '0 ' + str(centroid_x/640)[:6] \
                   + ' ' + str(centroid_y/480)[:6] \
                   + ' ' + str(bw/480)[:6] \
                   + ' ' + str(bh/480)[:6]
...
if __name__ == '__main__':
    ...
    position_np = np.load('position1.npy')
    ...
    images_folder = 'H:/data/fly_coco_yolo_4'
    label_path='H:/tracking/yolov5/data/fly/labels/train2017'
    creat_txt_for_yolov5(position_np, images_folder, label_path)
